...
{   
    ...
    protected function setHttpAuthorization()
    {
        if (isset($_SERVER['HTTP_AUTHORIZATION']))
            ...
            if (isset($_SESSION[DB_NAME]) && isset($_SESSION[DB_NAME]['token']) && $_SESSION[DB_NAME]['token']!='')
               $this->http_autorization = $_SESSION[DB_NAME]['token'];
    ...
    protected function setFilters()
    {
        ...
        if ($this->request)
        {
            if (strpos($this->request, '/') === false) // if only one filter is the module
                self::$module_name = strtolower($this->request);
            else
            {
                $filt = explode('/', $this->request);
                
                self::$module_name = strtolower(array_shift($filt));
